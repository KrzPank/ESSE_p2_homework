...
'''
def simulate(J, dt=0.1, T=100.0, sp_deg=10.0):
  # Satellite setup
  sat = satellite(j=J, name=f"sat_J_{J}")
  sat_gen = sat.generator(dt, ic=[0.0, 0.0])
  sat_gen.send(None)  # Start satellite generator

  # Controller setup
  ctrl = contr(name="Controller")
  ctrl_gen = ctrl.generator(dt, ic=0.0)
  ctrl_gen.send(None)  # Start controller generator
  ...
  for idx, t in enumerate(time_vector):
    sat1 = sat_gen.send((t, cv))
    cv = ctrl_gen.send((t, u[idx], sat1, cv))

  plt.plot(time_vector, u, label='Input u(t)')
  sat.plot()
  plt.grid()
  fname_ctrl = f'homework/satellite_states.png'
  plt.savefig(fname_ctrl)
  plt.close()

  ctrl.plot()
  fname_ctrl = f'homework/step_response_ctrl_J{idx}.png'
  plt.savefig(fname_ctrl)
  plt.close()


simulate(J=10.8e8, dt=0.1, T=100.0, sp_deg=10.0)'''